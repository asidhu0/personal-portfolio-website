...
function Projects() { 
    const dcDescription = 'Developed the UC Davis Dining Hall App using the MERN stack while mentoring a cross-functional team of developers and designers. The app displays menus, offers dietary-based filtering, tracks calories, and saves favorite food items.'
    const grataDescription = 'Built an iOS native application with five other developers. The app allows building managers to easily pair Bluetooth locks with tenant units, lock and unlock units, change lock PINs, unpair locks, and manage other minor account-related features.'
    const labSchedulerDescription = 'Implemented software to display live server data and availability status using host/client scripts with TCP sockets. This enables engineers to monitor server availability and reserve it accordingly via Microsoft Teams.'
    const userThreadLibraryDescription = 'Developed a Linux-based user-level thread library, providing an interface for simulating multi-threading and concurrency. Implemented user-thread, semaphore, and preemption API\'s for efficient thread management and synchronization.'
    const tuneHuntDescription = 'Implemented an interactive, map-based game using React Native. Users can collect album covers based on their location, add them to their library, and link to the Spotify page to discover new music!'
    ...
    const terminalDescription = 'Developed a custom shell that handles basic built-in commands, supports multi-piping and output redirection, and manages simple environment variables. Ensured efficient command parsing with error handling and proper memory management.'
    ...
    const ticTacToeDetails = `
        ...
        - Added animations for X and O pieces and alerts for wins, losses, and draws to enchance the user experience
        `
